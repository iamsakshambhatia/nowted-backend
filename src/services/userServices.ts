...
export const getAllUsersService = async () => {
  const result = await pool.query(
    "SELECT id, username, email FROM users WHERE archived_at IS NULL"
  );
  return result.rows;
};
...
export const deleteUserService = async (id: string) => {
  const result = pool.query("UPDATE users SET archived_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id, username, email", [
    id,
  ]);
...
export const restoreUserService = async (id: string) => {
  const result = pool.query(
    "UPDATE users SET archived_at = null WHERE id = $1 RETURNING id, username, email",
    [id]
  );
  return (await result).rows[0];
};
